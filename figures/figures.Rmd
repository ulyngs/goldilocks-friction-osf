...
```{r}
### try and draw the apple one again with color
...
save_plot(here::here("figures", "apple_ratings_and_features.png"), test_apple,
...
boxplot_ratings <- meta_data %>%
  ...
  ggplot(aes(x = categories_alphabetically, y = average_rating)) +
    ...
    geom_jitter(width = 0.3, height = 0.1, size = 1.1, alpha = 0.7) +
    theme_minimal() +
    labs(x = "", y = "") +
    coord_flip() +
