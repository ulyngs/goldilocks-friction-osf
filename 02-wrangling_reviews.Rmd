...
all_reviews <- chrome_reviews %>% 
  filter(type == "review") %>% 
  select(tool_id = extension_url, user_name, date = date_modified, rating, review_text = text, store) %>% 
...
all_reviews_distinct %>%
  ...
  mutate(tool_id = if_else(store == "chrome", 
                           str_remove(tool_id, "\\?hl=.+") %>%  str_extract("/[^/]+$") %>% str_remove("/"),
                           tool_id)) %>% 
